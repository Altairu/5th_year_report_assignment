...
typedef RT_MODEL_ex_picont_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * ex_picont(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void ex_picont_initialize();
   extern void ex_picont_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(ex_picont_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://ex_picont:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: ex_picont -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
