...
typedef RT_MODEL_cart_count_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * cart_count(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void cart_count_initialize();
   extern void cart_count_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(cart_count_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://cart_count:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: cart_count -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
