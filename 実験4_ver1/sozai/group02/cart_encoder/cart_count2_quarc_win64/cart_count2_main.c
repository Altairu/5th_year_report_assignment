...
typedef RT_MODEL_cart_count2_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * cart_count2(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void cart_count2_initialize();
   extern void cart_count2_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(cart_count2_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://cart_count2:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: cart_count2 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
