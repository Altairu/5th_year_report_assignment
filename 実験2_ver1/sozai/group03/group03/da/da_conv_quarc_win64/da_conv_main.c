...
typedef RT_MODEL_da_conv_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * da_conv(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void da_conv_initialize();
   extern void da_conv_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(da_conv_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://da_conv:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: da_conv -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
