...
typedef RT_MODEL_pend_count3_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * pend_count3(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void pend_count3_initialize();
   extern void pend_count3_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(pend_count3_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://pend_count3:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: pend_count3 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
